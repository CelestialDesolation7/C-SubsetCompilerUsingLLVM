...
当 `add`/`sub` 的一个操作数为立即数时，直接生成 `addi`：
...
├─────────────────┤
│  callee-saved    │ s1, s2, ... (若有)
├─────────────────┤
│  局部变量 (alloca) │ -4(s0), -8(s0), ...
├─────────────────┤
│  溢出栈槽 (spill)  │ offset(sp)
└─────────────────┘ ← sp
低地址
```
...
```bash
# 1. ToyC 生成汇编
./build/toyc.exe examples/compiler_inputs/01_minimal.c -o test/asm/01_minimal_toyc.s
